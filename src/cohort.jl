...
function user_cohort_table(case)
    ...
    return FunSQL.SQLTable(qualifiers = [:ctsi, Symbol(sname)], name = :cohort,
                           columns = [:study_id, :person_id, :added])
end
...
function funsql_to_study_id(case)
    ...
    @funsql begin
        left_join($name => from($cohort), $name.person_id == person_id)
...
function user_rebuild_cohort_table(db, case)
    ...
    DBInterface.execute(db, """DELETE FROM $name_sql WHERE person_id IN ($deletions)""")
